...
const Paso5 = props => {
		...
		if (formData.filesChanged === true) {
			const uploadFiles = async () => {
				...
				for (let i = 0; i < formData.linkDiagnostico.length; i++) {
					filesFormData.append(`Diagnóstico Médico - Archivo N${i}`, formData.linkDiagnostico[i]);
				}
				for (let i = 0; i < formData.dniPasaporte.length; i++) {
					filesFormData.append(`Pasaporte - Archivo N${i}`, formData.dniPasaporte[i]);
				}
				axiosInstance.post(`/inscripcion/subir-archivos/${formData.correoElectronico}`, filesFormData, {
		...
		if (cookies.get('id')) {
			try {
				...
				Swal.fire({
					icon: 'success',
					title: 'Perfecto!',
					text: 'Sus datos han sido actualizados y serán revisados nuevamente. Gracias!',
					onAfterClose: () => {
	...
	return (
		<div className='resultados'>
			<h2>Por favor, verifique que haya ingresado sus datos correctamente antes de enviar el formulario</h2>
			<ul>
				<li>
